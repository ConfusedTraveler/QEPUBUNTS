...
df_cnnlstm = pd.read_csv('cnnlstm_temp.csv')#, names=['Actual','Pred'])
# df_scinet = pd.read_csv('scinet_temperature.csv')

df_lstm = pd.read_csv('lstm_temp.csv')#, names=['Actual','Pred'])

# Read the CSV file into a DataFrame and manually assign column names
column_names = ['length', 'epsilon', 'pimax','H']
dfk = pd.read_csv('pimax_temp_NLZ2.csv', names=column_names, header=None)

dfcn = pd.read_csv('pimax_temp_NLZ1.csv', names=column_names, header=None)
...
plt.legend(["Pimax_LZ2","Pimax_LZ1","Pi_lstm","Pi_cnnlstm"],fontsize=22)
plt.tight_layout()

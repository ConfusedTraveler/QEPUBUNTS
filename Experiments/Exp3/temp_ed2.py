...
if __name__ == "__main__":
    ...
    if model == "arima":
        ...
        print("[进度] 开始ARIMA模型训练...")
        arima_predict.arima([n],series,output_path)
        print("[进度] ARIMA模型训练完成！")
    elif model == "lstm":
        ...
        pimax = []
        Hest = []
        total_steps = len(epsilon)
        print(f"[进度] 开始Pimax计算，使用估计器: {estimator}，总共{total_steps}个epsilon值...")
        ...
        if estimator == "NLZ1":
            for idx, e in enumerate(epsilon):
                ...
                Hest.append(H)
                print(f"[进度] 已完成 {idx+1}/{total_steps} (epsilon={e})")
        else:
            for idx, e in enumerate(epsilon):
                N = (max(series)-min(series)+2*e)/e
